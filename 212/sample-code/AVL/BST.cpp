...
void BinarySearchTree::rotateLeft()
{
	rotateLeftTree(root);
}

void BinarySearchTree::rotateLeftTree(TreeNode* & root) 
{
     TreeNode * oldRoot = root;

           // perform rotation
     root = root->rightChildPtr;
     oldRoot->rightChildPtr = root->leftChildPtr;
     root->leftChildPtr = oldRoot;
...
void BinarySearchTree::rotateRight()
{
	rotateRightTree(root);
}

void BinarySearchTree::rotateRightTree(TreeNode * & root) 
{
     TreeNode * oldRoot = root;

           // perform rotation
     root = root->leftChildPtr;
     oldRoot->leftChildPtr = root->rightChildPtr;
     root->rightChildPtr = oldRoot;
...
void BinarySearchTree::balanceTree(TreeNode * & tree)
{
	if (tree == NULL)
		return;
	int leftHeight = m.getHeight(tree->leftChildPtr,0);
	int rightHeight = m.getHeight(tree->rightChildPtr,0);
	...
	if (abs(leftHeight - rightHeight) > 1)
	{
		if (leftHeight > rightHeight)
		{
			if (tree->leftChildPtr == NULL)
				rotateRightTree(tree);
			balanceTree(tree->leftChildPtr);
			rotateRightTree(tree);
		}
		else
		{
			if (tree->rightChildPtr == NULL)
				rotateLeftTree(tree);
			balanceTree(tree->rightChildPtr);
			rotateLeftTree(tree);
		}
	}
}
