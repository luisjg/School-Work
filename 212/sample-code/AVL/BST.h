...
// constructors and destructor
  ...
  // Precondition for all methods: no two items in a binary search tree
  //    have the same search key
   ...
   * @post If the item whose search key equals searchKey existed in the
   *       tree, the item is deleted. Otherwise, the tree is unchanged.
  ...
  /** Traverses a binary search tree in sorted order, calling function
   *  visit() once for each item. */
  virtual void inorderTraverse(FunctionType visit);

  /** Traverses a binary search tree in postorder, calling function
   *  visit() once for each item. */
...
/** setMapParms
  In: needs the binary tree set up -- calls setMap()
  Out: setMap creates a two-dimensional array that has binary tree elements according
     to levels (height 0, 1, and so on) with root at height 0, two items at height 1
	 and so on.
  Will get height via getHeight and use it to create a two-dimensional array that maps out the
     tree items
  */
  void setMapParms();
  void print();
  ...
  void rotateRight();
  void rotateRightTree(TreeNode * & root);
  void rotateLeft();
  void rotateLeftTree(TreeNode * & root);
  void balance();
  void balanceTree(TreeNode * & tree);

protected:
  /** Recursively inserts an item into a binary search tree.
   * @pre treePtr points to a binary search tree, newItem is the item
   *              to be inserted
   ...
   * @pre nodePtr point to a node in a binary search tree;
   *      nodePtr != NULL`
   * @post treeItem contains the item in the leftmost descendant of
   *       the node to which nodePtr points. The leftmost descendant
   *       of nodePtr is deleted. */
  ...
  // the following 9 methods are the same as for the ADT binary tree
